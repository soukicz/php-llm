...
class OpenAICompatibleClient extends OpenAIEncoder implements LLMBatchClient {
    ...
    public function __construct(
        ...
        private $customHttpMiddleware = null,
    ) {
    }
    ...
    private function getChatRequest(LLMRequest $request): RequestInterface {
        return new Request('POST', $this->baseUrl . '/chat/completions', [
    ...
    public function getBatchEmbeddings(array $texts, string $model = 'text-embedding-3-small', int $dimensions = 512): array {
        ...
        foreach (array_chunk($texts, 100, true) as $chunk) {
            ...
            $response = json_decode($this->getHttpClient()->post($this->baseUrl . '/embeddings', [
                'json' => [
                    'model' => $model,
    ...
    public function createBatch(array $requests): string {
        ...
        $fileResponse = $this->getHttpClient()->post($this->baseUrl . '/files', [
            'multipart' => [
        ...
        $batchResult = $this->getHttpClient()->post($this->baseUrl . '/batches', [
            'json' => [
    ...
    public function retrieveBatch(string $batchId): ?array {
        $response = json_decode($this->getHttpClient()->get($this->baseUrl . '/batches/' . $batchId)->getBody(), true, 512, JSON_THROW_ON_ERROR);
        ...
        if ($response['output_file_id'] === null && $response['error_file_id']) {
            ...
            $file = (string) $this->getHttpClient()->get($this->baseUrl . '/files/' . $response['error_file_id'] . '/content')->getBody();

            throw new \RuntimeException('Batch failed: ' . substr($file, 0, 1000));
        }

        $file = (string) $this->getHttpClient()->get($this->baseUrl . '/files/' . $response['output_file_id'] . '/content')->getBody();
        $results = explode("\n", trim($file));
        $responses = [];
